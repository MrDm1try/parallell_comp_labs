...
function execute() {
    ...
    if [[ "${EXECUTABLE}" != "${EXECUTABLE_SEQ}" ]]; then
...
do
    ...
    execute ${EXECUTABLE_OMP} ${N} ${OMP_TIME_LINE_NUM_START} ${OMP_X_LINE_NUM}
    execute ${EXECUTABLE_PTHREADS} ${N} ${PTHREADS_TIME_LINE_NUM_START} ${PTHREADS_X_LINE_NUM}
    echo ${N}
done
